...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[dbasp_Backup_Cleanup2] (@BkUpPath varchar(100) = null
					,@Retention_DD int = 7
					,@BkUpType varchar(10) = 'tlog'
					,@LiteSpeed_Bypass char(1) = 'n'
					,@RedGate_Bypass char(1) = 'n'
					,@dup_delete char(1) = 'n'
					,@dup_DBname sysname = null)
 ...
 **
 **  @dup_DBname 	- DBname for the @dup_delete request (e.g. wcds)
 **
...
--  Note:  This process will delete all but the most recent backup files.                                      --
-----------------------------------------------------------------------------------------------------------------
dup_delete_start:
...
If rtrim(@BkUpPath) not like '%\'
   begin
	Select @BkUpPath = rtrim(@BkUpPath) + '\'
   end


--  Use the dbasp_FileHandler to remove all but the most recent backup files
;WITH BF
	AS
	(
	SELECT *
	FROM dbaudf_BackupScripter_GetBackupFiles(@dup_DBname,@BkUpPath,0,null)
	)
SELECT @Data = (
...
GO
GRANT EXECUTE ON  [dbo].[dbasp_Backup_Cleanup2] TO [public]
GO
