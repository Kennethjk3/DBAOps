...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[BlockMonitor]
(
...
+ ''nt_username'' + char(9) + ''loginame'' + char(9) + ''stmt_start'' + char(9) + ''stmt_end'' ; SELECT CAST(spid AS NVARCHAR) + char(9) + CAST(Chain AS NVARCHAR) + char(9) + CAST(blocked AS NVARCHAR) + char(9) + CAST(waittime AS NVARCHAR) + char(9) + CAST(lastwaittype AS NVARCHAR) + char(9) + CAST(uid AS NVARCHAR) + char(9) + CAST(cpu AS NVARCHAR) + char(9) + CAST(physical_io AS NVARCHAR) + char(9) + CAST(memusage AS NVARCHAR) + char(9) + CAST(login_time AS NVARCHAR) + char(9) + CAST(last_batch
 AS NVARCHAR) + char(9) + CAST(open_tran AS NVARCHAR) + char(9) + CAST(status AS NVARCHAR) + char(9) + CAST(hostname AS NVARCHAR) + char(9) + CAST(program_name AS NVARCHAR) + char(9) + CAST(cmd AS NVARCHAR) + char(9) + CAST(nt_domain AS NVARCHAR) + char(9) + CAST(nt_username AS NVARCHAR) + char(9) + CAST(loginame AS NVARCHAR) + char(9) + CAST(stmt_start AS NVARCHAR) + char(9) + CAST(stmt_end AS NVARCHAR) from ##BlockingChain order by Chain; SELECT CHAR(10) + CHAR(13) + CHAR(10) + CHAR(13); SELEC
T ''SPID'' + CHAR(9) + ''DatabaseName'' + CHAR(9) + ''request_status'' + CHAR(9) + ''request_mode'' + CHAR(9) + ''resource_type'' + CHAR(9) + ''LockedObjectID'' + CHAR(9) + ''LockedObjectName'' ; SELECT CAST(SPID AS NVARCHAR(MAX)) + CHAR(9) + DatabaseName + CHAR(9) + CAST(request_status AS NVARCHAR(MAX)) + CHAR(9) + CAST(request_mode AS NVARCHAR(MAX)) + CHAR(9) + resource_type + CHAR(9) + CAST(LockedObjectID AS NVARCHAR(MAX)) + CHAR(9) + CAST(LockedObjectName AS NVARCHAR(MAX)) FROM ##TableLocks
; SELECT CHAR(10) + CHAR(13) + CHAR(10) + CHAR(13) ; SELECT ''SPID'' + char(9) + ''DatabaseName'' + char(9) + ''SQLText'' ; SELECT CAST(spid AS NVARCHAR) + char(9) + CAST(DatabaseName AS NVARCHAR) + CHAR(9) + CAST(SQLText AS VARCHAR(4000)) from ##BlockingChain ;   '
    ...
    AND         EXISTS  (   SELECT        *
                            ...
                            AND         R2.cmd						NOT LIKE '%BACKUP DATABASE%'
    ...
    BEGIN
        ...
        BEGIN
            ...
            INTO        ##BlockingChain
            FROM        sys.sysprocesses            R   WITH (NOLOCK)
            JOIN        @HeadBlocker                W                   ON          W.SPID  =   R.blocked
            JOIN        sys.sysdatabases            D   WITH (NOLOCK)   ON          R.dbid  =   D.dbid
            ...
            ,           LEFT(ST.text, 8000)         AS SQLText
            FROM        sys.sysprocesses            R   WITH (NOLOCK)
            JOIN        @HeadBlocker                W                   ON          W.SPID  =   R.spid
            JOIN        sys.sysdatabases            D   WITH (NOLOCK)   ON          R.dbid  =   D.dbid
            CROSS APPLY sys.dm_exec_sql_text(R.sql_handle) ST
...
END /* Proc */
GO
GRANT EXECUTE ON  [dbo].[BlockMonitor] TO [public]
GO
