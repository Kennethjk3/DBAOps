...
CREATE   PROCEDURE [dbo].[dbasp_GetAgentScheduleTimes]
(
...
--                     scheduleEnabled
--              )
--SELECT        sj.originating_server_id,
--              sj.name,
...
--ORDER BY      sj.originating_server_id,
--              sj.name,
--              ss.name
...
SELECT        js.scheduleID,
              ...
              js.scheduleName,
              js.categoryName,
              CAST(st.infoDate AS Date) [StartDate],
              CAST(st.startTime AS Time) [StartTime],
              CAST(st.endTime AS Time) [EndTime],
              js.jobEnabled,
              js.scheduleEnabled
FROM          #scheduleTimes AS st
...
WHERE		  js.jobEnabled = 1
	...
	AND		  js.jobName NOT IN ('MAINT - Rapid Stats Updates','UTIL - DBA Daily 5min Processing','MAINT - TranLog Backup','UTIL - PERF Daily 5min Processing'
									,'','','','','','','','','','','','','','','')
