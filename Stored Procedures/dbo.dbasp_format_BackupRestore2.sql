...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[dbasp_format_BackupRestore2]
						(
						@DBName			SYSNAME		= NULL
...
--	======================================================================================
	...
	BEGIN	-- VARIABLE AND TEMP TABLE DECLARATIONS
		...
		IF OBJECT_ID('tempdb..#FLX') IS NOT NULL
			DROP TABLE #FLX


		IF OBJECT_ID('tempdb..#Fgrps') IS NOT NULL
			DROP TABLE #Fgrps
		...
		IF OBJECT_ID('tempdb..#FLst') IS NOT NULL
			DROP TABLE #FLst


		IF OBJECT_ID('tempdb..#FLst_Last') IS NOT NULL
			DROP TABLE #FLst_Last
	...
	BEGIN	-- VARIABLE INITIALIZATIONS AND PARAMETER CHECKING
		...
		SELECT	 	@NOW			= REPLACE(REPLACE(REPLACE(CONVERT(VarChar(50),getdate(),120),'-',''),':',''),' ','')
				,@DataPath		= DBAOps.[dbo].[dbaudf_GetSharePath2]('mdf')
				,@NdfPath		= COALESCE	(
									DBAOps.[dbo].[dbaudf_GetSharePath2]('ndf')
									,@DataPath
									)
				,@LogPath	 	= DBAOps.[dbo].[dbaudf_GetSharePath2]('ldf')
				,@FilePath	 	= COALESCE	(
									@FilePath
									,DBAOps.[dbo].[dbaudf_GetSharePath2]('backup')
									,DBAOps.[dbo].[dbaudf_getShareUNC]('backup')
									)
		...
		If @Compression = 1
		BEGIN
			IF @DBName IN ('master','model','msdb','temp','DBAOps','dbaperf','DBAOps')
				SET @Compression = 0
	...
	IF @EvaluateFilesAt IS NOT NULL
	BEGIN
		-- ADD DYNAMIC LINKED SERVER
		RAISERROR('Generating Linked Server to the Server %s.',-1,-1,@EvaluateFilesAt) WITH NOWAIT
		...
		EXEC ('sp_addlinkedserver @server=''DYN_DBA_RMT'',@srvproduct='''',@provider=''SQLNCLI'',@datasrc=''tcp:'+@EvaluateFilesAt+'''')
		...
		EXEC ('master.dbo.sp_addlinkedsrvlogin @rmtsrvname=N''DYN_DBA_RMT'',@useself=N''FALSE'',@locallogin=NULL,@rmtuser=''LinkedServer_User'',@rmtpassword=''${{secrets.LINKEDSERVER_USER_PW}}''')
	...
	BEGIN	-- SCRIPT BACKUPS
		...
		BEGIN
			...
			SET		@TBL = [DBAOps].[dbo].[dbaudf_FormatString](@FMT1,@ColName1,@ColName2,@ColName3,@ColName4,@ColName5,@ColName6,@ColName7,@ColName8,@ColName9,'') +@CRLF
					+ @HeaderLine +@CRLF
			SELECT		@TBL = @TBL + [DBAOps].[dbo].[dbaudf_FormatString](@FMT2,DriveLetter,SizeGB,AvailableGB,FreeGB,DriveType,FileSystem,IsReady,VolumeName,RootFolder,'') +@CRLF
			FROM		#TMP1
		...
		SELECT		id
				,name
		FROM		#FGs
		WHERE		name in  (SELECT SplitValue FROM [DBAOps].[dbo].[dbaudf_StringToTable](@filegroups,','))
			OR	@filegroups = 'ALL'
		UNION ALL
		SELECT		1
				,NULL
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						exec DBAOps.dbo.dbasp_Estimate_Diff_Size @DBName = @DBname, @diffPrediction = @diffPrediction output
				...
				SELECT	@CMD		= CASE @Mode
					...
					,@SetNumber	= 0
					,@FileName	= COALESCE	(
									REPLACE(@ForceFileName,'$FG$',COALESCE(@FileGroup,''))
									,@DBName
										+ CASE
				...
				SELECT        @CMD	= @CMD
							+ ' WITH '
							+ DBAOps.dbo.dbaudf_ConcatenateUnique (WithOptions)
				FROM		(
		...
		BEGIN	-- GATHER LIST OF BACKUP FILES
			...
			BEGIN TRY
				...
				DECLARE	@DBName			SYSNAME		= '''+@DBName+'''
						,@FilePath		VarChar(max)	= '''+@FilePath+'''
						,@IncludeSubDir	BIT			= '+ CAST(@IncludeSubDir AS Char(1))+'
						,@ForceFileName	VarChar(max)	= '+COALESCE(QUOTENAME(@ForceFileName,''''),'NULL')+'
				SELECT		*
				FROM		DBAOps.dbo.dbaudf_BackupScripter_GetBackupFiles(@DBName,@FilePath,@IncludeSubDir,@ForceFileName)'


				IF @EvaluateFilesAt IS NULL
				BEGIN
					INSERT INTO	@SF
					EXEC(@CMD)
				END
			...
			BEGIN CATCH
				...
				RAISERROR ('USING : SELECT * FROM DBAOps.dbo.dbaudf_BackupScripter_GetBackupFiles(''%s'',''%s'',''%d'',''%s'')',-1,-1,@DBName,@FilePath,@TempInt,@ForceFileName) WITH NOWAIT
...
--SELECT * FROM @SF ORDER BY [BackupTimeStamp] DESC
		...
		UPDATE		T1
			SET	BackupTimeStamp = (SELECT MIN([DBAOps].[dbo].[dbaudf_GetFileProperty]([FullPathName],'file','CreationTime')) FROM @SF WHERE Mask = T1.Mask)
				,BackupType = 'DB'
		FROM		@SF T1
		WHERE		BackupType = '??'
...
--SELECT * FROM @SF ORDER BY [BackupTimeStamp] DESC
		...
		BEGIN	-- CHECK BACKUP FILES
			...
			--AND		Name Not In	(
			--				SELECT		DISTINCT
			--						[DBAOps].[dbo].[dbaudf_GetFileFromPath]([physical_device_name]) [Name]
			--				FROM		msdb.dbo.restorehistory rh
			--				JOIN		[msdb].[dbo].[backupset] bs
			--					ON	rh.backup_set_id = bs.backup_set_id
			--				JOIN		[msdb].[dbo].[backupmediafamily] bmf
			...
			BEGIN
				...
				BEGIN
					...
					IF @Verbose >= 1
						RAISERROR('    -- Calling DBAOps.dbo.dbaudf_BackupScripter_GetFileList (''%s'',''%s'',%d,''%s'',''%s'',@OverrideXML,''%s'',''%s'',''%s'',''%s'')',-1,-1,@DBName,@NewDBName,@SetSize,@FileName,@FullPathName,@NOW,@DataPath,@NdfPath,@LogPath) WITH NOWAIT


					BEGIN TRY
						SET @CMD = '
						DECLARE	@DBName			SYSNAME		= '''+@DBName+'''
								,@NewDBName		SYSNAME		= '''+@NewDBName+'''
								,@SetSize			INT			= '+ CAST(@SetSize AS VarChar(2))+'
								,@FileName		VarChar(max)	= '''+@FileName+'''
								,@FullPathName		VarChar(max)	= '''+@FullPathName+'''
								,@OverrideXML		XML			= '+COALESCE(QUOTENAME(DBAOps.dbo.dbaudf_FormatXML2String(@OverrideXML),''''),'NULL')+'
								,@NOW			VARCHAR(20)	= '''+@NOW+'''
								,@DataPath		VarChar(max)	= '''+@DataPath+'''
								,@NdfPath			VarChar(max)	= '''+@NdfPath+'''
								,@LogPath			VarChar(max)	= '''+@LogPath+'''


						SELECT	*
						FROM		DBAOps.dbo.dbaudf_BackupScripter_GetFileList (@DBName,@NewDBName,@SetSize,@FileName,@FullPathName,@OverrideXML,@NOW,@DataPath,@NdfPath,@LogPath)'


						--exec DBAOps.dbo.dbasp_PrintLarge @CMD
					...
					IF @Verbose >= 1
						RAISERROR('    -- Calling DBAOps.dbo.dbaudf_BackupScripter_GetHeaderList (%d,''%s'',''%s'')',-1,-1,@SetSize,@FileName,@FullPathName) WITH NOWAIT

					BEGIN TRY


						SET @CMD = '
						DECLARE	@SetSize			INT			= '+ CAST(@SetSize AS VarChar(2))+'
								,@FileName		VarChar(max)	= '''+@FileName+'''
								,@FullPathName		VarChar(max)	= '''+@FullPathName+'''


						SELECT	*,null,null,null,null
						FROM		DBAOps.dbo.dbaudf_BackupScripter_GetHeaderList (@SetSize,@FileName,@FullPathName)'
...
--SELECT * FROM #Fgrps
			...
			BEGIN
				...
				INTO		#TMP5
				FROM		#Fgrps T1
				LEFT JOIN	[DBAOps].[dbo].[dbaudf_StringToTable](@filegroups,',') T2
				...
				BEGIN
					...
					SET		@TBL = [DBAOps].[dbo].[dbaudf_FormatString](@FMT1,@ColName1,@ColName2,@ColName3,@ColName4,@ColName5,@ColName6,@ColName7,@ColName8,@ColName9,'') +@CRLF
							+ @HeaderLine +@CRLF
					SELECT		@TBL = @TBL + [DBAOps].[dbo].[dbaudf_FormatString](@FMT2,FileGroupName,FileGroupID,HasFGExcluded,HasFGIncluded,BeingRestored,'','','','','') +@CRLF
					FROM		#TMP5
			...
			BEGIN	-- REMOVE FG BACKUPS FOR FG'S NOT BEING RESTORED
				...
				WHERE		Mask NOT IN	(
									SELECT		DISTINCT
											T1.BackupFileName
									FROM		@FL T1
									JOIN		[DBAOps].[dbo].[dbaudf_StringToTable](@filegroups,',') T2
											ON	T1.FileGroupName = T2.SplitValue
											AND	T1.IsPresent = 1
									)
...
--SELECT * FROM @RC ORDER BY FirstLSN DESC
				---------------------------------------------------------------------------------
				---------------------------------------------------------------------------------
...
--SELECT * FROM @SF
		...
		BEGIN	-- REPORT BACKUP FILE AND HEADER INFO
			...
			BEGIN
				...
				SET		@TBL = [DBAOps].[dbo].[dbaudf_FormatString](@FMT1,@ColName1,@ColName2,@ColName3,@ColName4,@ColName5,@ColName6,@ColName7,@ColName8,@ColName9,'') +@CRLF
						+ @HeaderLine +@CRLF
				SELECT		@TBL = @TBL + [DBAOps].[dbo].[dbaudf_FormatString](@FMT2,Name,FromServer,Type,Engine,SetSize,Files,Size_GB,Size,'','') +@CRLF
				FROM		#TMP2
			...
			BEGIN
				...
				SET		@TBL = [DBAOps].[dbo].[dbaudf_FormatString](@FMT1,@ColName1,@ColName2,@ColName3,@ColName4,@ColName5,@ColName6,@ColName7,@ColName8,@ColName9,'') +@CRLF
						+ @HeaderLine +@CRLF
				SELECT		@TBL = @TBL + [DBAOps].[dbo].[dbaudf_FormatString](@FMT2,Name,FirstLSN,LastLSN,DatabaseBackupLSN,'','','','','','') +@CRLF
				FROM		#TMP3
			...
			BEGIN
				...
				SET		@TBL = [DBAOps].[dbo].[dbaudf_FormatString](@FMT1,@ColName1,@ColName2,@ColName3,@ColName4,@ColName5,@ColName6,@ColName7,@ColName8,@ColName9,'') +@CRLF
						+ @HeaderLine +@CRLF
				SELECT		@TBL = @TBL + [DBAOps].[dbo].[dbaudf_FormatString](@FMT2,Name,BackupTimeStamp,BackupStartDate,BackupFinishDate,BackupDateRange_Start,BackupDateRange_End,BackupChainStartDate,BackupLinkStartDate,'','') +@CRLF
				FROM		#TMP4
...
--RAISERROR('A2',-1,-1) WITH NOWAIT
			...
			SELECT		LogicalName
					...
					,NEW_PhysicalName
					,MAX(CASE [DBAOps].[dbo].[dbaudf_GetFileProperty](NEW_PhysicalName,'FILE','Exists') WHEN 'True' THEN 1 ELSE 0 END) [FileExists]
					,MAX(CASE [DBAOps].[dbo].[dbaudf_GetFileProperty](NEW_PhysicalName,'FILE','Exists') WHEN 'True' THEN CAST(CAST([DBAOps].[dbo].[dbaudf_GetFileProperty](NEW_PhysicalName,'FILE','Length') AS NUMERIC(38,10))/CAST(POWER(1024.0,3) AS NUMERIC(38,10)) AS NUMERIC(38,10)) ELSE 0 END) [ExistingSizeGB]
			...
			FROM		#FLX
			LEFT JOIN	DBAOps.dbo.dbaudf_ListDrives() Drives
				ON	Drives.RootFolder = [DBAOps].[dbo].[dbaudf_GetFileProperty](NEW_PhysicalName,'file','RootFolder')
			WHERE		IsPresent = 1
				AND	(type = 'L' OR @filegroups IS NULL OR FileGroupName IN (Select SplitValue FROM [DBAOps].[dbo].[dbaudf_StringToTable](@filegroups,',')))
...
--SELECT * FROM #DBFileSpaceCheck
			...
			BEGIN
				...
				SET		@TBL = [DBAOps].[dbo].[dbaudf_FormatString](@FMT1,@ColName1,@ColName2,@ColName3,@ColName4,@ColName5,@ColName6,@ColName7,@ColName8,@ColName9,'') +@CRLF
						+ @HeaderLine +@CRLF
				SELECT		@TBL = @TBL + [DBAOps].[dbo].[dbaudf_FormatString](@FMT2,LogicalName,DriveLetter,New_PhysicalName,FileExists,ExistingSizeGB,DriveFreeGB,SizeGB,MaxSizeGB,'','') +@CRLF
...
--SELECT * FROM @FL
			...
			BEGIN		-- COPY FILES TO LOCAL WORK DIRECTORY AND RESTORE FROM THERE
				...
				SELECT		@CMD =	[DBAOps].[dbo].[dbaudf_FormatXML2String]((
								SELECT		*
			...
			----------------------------------------------------------------------------------------------------
			----------------------------------------------------------------------------------------------------
			IF @FullReset = 1 -- DROP EXISTING DATABASE AND DELETE BACKUP HISTORY
			BEGIN
				SET @syntax_out	= COALESCE(@syntax_out,'')
						+ @CRLF
			...
			SET @syntax_out	= COALESCE(@syntax_out,'')
					+ @CRLF
					+ 'DECLARE @FilesRestored INT = 0' + @CRLF
					+ @CRLF
			...
			BEGIN
				...
				BEGIN
					...
					IF @BackupType NOT IN ('DB','FG') and DB_ID(@NewDBName) IS NOT NULL
						INSERT INTO	#FLst_Last
						SELECT		*
						FROM		#FLst
					...
					BEGIN
						IF @WorkDir IS NOT NULL
							SELECT		@FileNameSET = REPLACE(DBAOps.dbo.dbaudf_ConcatenateUnique('DISK = '''+@WorkDir + T1.Name+''''+ @CRLF),',','       ,')
							FROM		DBAOps.dbo.dbaudf_DirectoryList2(@FilePath,NULL,@IncludeSubDir) T1
							WHERE		T1.Name LIKE @FileName
						ELSE
							SELECT		@FileNameSET = REPLACE(DBAOps.dbo.dbaudf_ConcatenateUnique('DISK = '''+T1.FullPathName+''''+ @CRLF),',','       ,')
							FROM		DBAOps.dbo.dbaudf_DirectoryList2(@FilePath,NULL,@IncludeSubDir) T1
							WHERE		T1.Name LIKE @FileName
					END
					ELSE
				...
				SELECT        @CMD	= @CMD
							+ '  WITH  '
							+ REPLACE(DBAOps.dbo.dbaudf_ConcatenateUnique (WithOptions),',',@CRLF+'        ,')
							--+ @CRLF
				FROM		(
				...
				--PRINT '-----------------------------------'
						...
						BEGIN
							SELECT		@CMD = @CMD
									+ ISNULL(@CRLF+'        ,'+NULLIF(REPLACE(DBAOps.dbo.dbaudf_Concatenate ('MOVE ''' + LogicalName + ''' TO ''' + NEW_PhysicalName + ''''),',',@CRLF+'        ,'),''),'')
							FROM		#FLst
							WHERE		isPresent = 1
								AND	(type = 'L' OR @filegroups IS NULL OR FileGroupName IN (Select SplitValue FROM [DBAOps].[dbo].[dbaudf_StringToTable](@filegroups,',')))
						END
				...
				--PRINT '-----------------------------------'
					...
					BEGIN
						...
						FROM		#FLst T1
						WHERE		isPresent = 1
							AND	FileGroupName IN (Select SplitValue FROM [DBAOps].[dbo].[dbaudf_StringToTable](@filegroups,','))
							AND	NOT EXISTS(SELECT * FROM #FLst_Last WHERE isPresent = 1 AND LogicalName = T1.LogicalName)
						ORDER BY	T1.FileID
					END
			...
			--PRINT @CMD
					IF @UseTryCatch = 1
						SET @CMD	= 'BEGIN TRY' + @CRLF
								...
								+ 'BEGIN CATCH' + @CRLF
								+ '	EXEC DBAOps.dbo.dbasp_GetErrorInfo' + @CRLF
								+ 'END CATCH' + @CRLF
								+ @CRLF
	...
	END
GO
GRANT EXECUTE ON  [dbo].[dbasp_format_BackupRestore2] TO [public]
GO
