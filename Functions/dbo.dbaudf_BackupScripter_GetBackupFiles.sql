...
SET ANSI_NULLS ON
GO
CREATE   FUNCTION [dbo].[dbaudf_BackupScripter_GetBackupFiles]
				(
				@DBName				SYSNAME			--= 'MDI'
...
(
	...
	FROM		DBAOps.dbo.dbaudf_DirectoryList2(
												COALESCE(NULLIF(REPLACE(@ForceFileName,[DBAOps].[dbo].[dbaudf_GetFileFromPath](@ForceFileName),''),''),@FilePath)
												,COALESCE([DBAOps].[dbo].[dbaudf_GetFileFromPath](@ForceFileName),@DBName+'*')
												,@IncludeSubDir
												) T1
	WHERE		REPLACE
				(
